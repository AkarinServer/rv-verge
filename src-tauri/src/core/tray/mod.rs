    ...
    async fn get_common_tray_icon(verge: &IVerge) -> (bool, Vec<u8>) {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if tray_icon_colorful == "monochrome" {
                // macOS 优先使用 PNG 格式，显示效果更好
                (
                    false,
                    include_bytes!("../../../icons/tray-icon-mono.png").to_vec(),
                )
            } else {
                (
                    false,
    ...
    async fn get_sysproxy_tray_icon(verge: &IVerge) -> (bool, Vec<u8>) {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if tray_icon_colorful == "monochrome" {
                ...
                (
                    false,
                    include_bytes!("../../../icons/tray-icon-sys-mono-new.png").to_vec(),
                )
            } else {
                (
                    false,
                    include_bytes!("../../../icons/tray-icon-sys.png").to_vec(),
                )
            }
        }
    ...
    async fn get_tun_tray_icon(verge: &IVerge) -> (bool, Vec<u8>) {
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if tray_icon_colorful == "monochrome" {
                ...
                (
                    false,
                    include_bytes!("../../../icons/tray-icon-tun-mono-new.png").to_vec(),
                )
            } else {
                (
                    false,
                    include_bytes!("../../../icons/tray-icon-tun.png").to_vec(),
                )
            }
        }
        #[cfg(not(target_os = "macos"))]
