    ...
    pub fn create_window(is_show: bool) -> Pin<Box<dyn Future<Output = bool> + Send>> {
        Box::pin(async move {
            ...
            if let Some(existing_window) = app_handle.get_webview_window("main") {
                logging!(info, Type::Window, "窗口已存在，直接显示");
                if is_show {
                    if let Err(e) = existing_window.show() {
                        logging!(error, Type::Window, "显示已存在窗口失败: {}", e);
                        return false;
                    }
                    if let Err(e) = existing_window.set_focus() {
                        logging!(error, Type::Window, "聚焦窗口失败: {}", e);
                    }
                }
            ...
            if !is_show {
                logging!(info, Type::Window, "静默启动，不创建窗口");
                return false;
            }
            ...
            if WindowOperationResult::Failed == Self::show_main_window().await {
                logging!(error, Type::Window, "显示主窗口失败");
                return false;
            }
