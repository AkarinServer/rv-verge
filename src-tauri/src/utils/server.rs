...
pub async fn check_singleton() -> Result<()> {
    ...
    let port_check = tokio::time::timeout(
        ...
        tokio::task::spawn_blocking(move || local_port_available(port))
    ).await;
    
    let port_available = match port_check {
        Ok(Ok(available)) => available,
        Ok(Err(e)) => {
            logging!(warn, Type::Setup, "端口检查失败: {}, 假设端口可用", e);
            true // 如果检查失败，假设端口可用，继续启动
        }
        Err(_) => {
            logging!(warn, Type::Setup, "端口检查超时，假设端口可用");
            true // 超时则假设端口可用
        }
    };
